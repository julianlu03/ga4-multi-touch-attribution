...
Don't blindly trust last-click. If you're only giving credit to the channel or source your customer came from right before purchase, you might be skipping much of the journey it took to get them there in the first place. In this project, I challenge this assumption: I extracted three months of Google Merchandise Store data from BigQuery and built three different attribution models to see how each one represented channel contribution.
...
Before doing anything else, I had to decide whether building multiple attribution models was even worthwhile. If the large majority of conversions in this three-month period were single-touch (only one session before conversion), the results of every model would look almost identical to Last-Touch. Before extracting the full dataset, I queried session counts per purchaser directly in BigQuery to build a histogram of conversions against number of pre-purchase sessions. With 2,084 (47%) single-session purchasers and 2,335 (53%) multi-session purchasers, this confirmed there was enough multi-touch behavior in the data to make attribution modeling meaningful.
...
**![Model comparison, multi-touch only](images/model_comparison_multitouch_final.png)**
...
For a retailer with this purchase pattern, with roughly half of customers converting in a single session, model choice is close to irrelevant for that half, since every model assigns the same credit when there's only one touchpoint to assign it to. The difference between models matters specifically for customers with longer, multi-touch consideration journeys. A business like this realistically shouldn't overhaul its entire attribution stack, since that's a lot of operational cost for a change that doesn't even affect half its customers. A more targeted approach: since Linear costs nothing to compute even where it's redundant, it can reasonably run as the default across all customers. The selectivity that matters is in where the organization spends attention acting on the result, revisiting budget decisions specifically for the channels where multi-touch credit meaningfully diverges from last-touch, not treating this as a reason to overhaul reporting company-wide.
